...
export class PerformanceController {
  ...
  @ApiOperation({
    ...
    description:
      'Guarda los resultados del examen en la base de datos y envía un informe por email al administrador',
  })
  @ApiResponse({
    status: 201,
  ...
  async submitExamResult(
    @Body() examData: SubmitPerformanceExamDto,
    @Request() req: any
  ) {
    this.logger.log(
    ...
    try {
      const result = await this.performanceService.submitExamResult(
        examData,
        userId
      );
      return result;
  ...
  @Get('results')
  @ApiOperation({
    summary: 'Obtener todos los resultados de exámenes de Performance',
  })
  @ApiQuery({
    name: 'githubProfile',
    required: false,
    description: 'Filtrar por perfil de GitHub',
  })
  @ApiQuery({
    name: 'passed',
    required: false,
  ...
  @ApiQuery({
    name: 'examPurpose',
    required: false,
    description: 'Filtrar por motivo del examen',
  })
  ...
  @Get('stats')
  @ApiOperation({
    summary: 'Obtener estadísticas generales de exámenes de Performance',
  })
  @ApiResponse({
    status: 200,
