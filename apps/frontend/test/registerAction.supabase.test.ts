...
function makeSupabaseMock(signUpResult: object) {
  ...
  // createClient es async en la versión actual
...
describe('registerAction — integración Supabase', () => {
  ...
  describe('candidato', () => {
    it('llama signUp con audience candidato y sin company_name ni ruc', async () => {
    ...
    it('NO incluye role para roles no especiales (qa_junior)', async () => {
      const { signUp } = makeSupabaseMock({ error: null });
      await registerAction(makeFormData(BASE_CANDIDATO));
    ...
    it('NO incluye role para roles no especiales (qa_senior)', async () => {
      const { signUp } = makeSupabaseMock({ error: null });
  ...
  describe('defaults', () => {
    it('usa audience "candidato" cuando no viene en el FormData', async () => {
      const { signUp } = makeSupabaseMock({ error: null });
      const fd = makeFormData({ email: 'x@test.com', password: 'Pass1234', name: 'X' });
  ...
  describe('resultados', () => {
    it('retorna { success: true, message } cuando Supabase no devuelve error', async () => {
    ...
    it('traduce "database error saving new user" a mensaje en español', async () => {
    ...
    it('traduce el error también cuando viene en minúsculas', async () => {
      makeSupabaseMock({ error: { message: 'database error saving new user' } });
      const result = await registerAction(makeFormData(BASE_CANDIDATO));
