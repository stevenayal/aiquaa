...
import { API_DEVELOPER_FUNDAMENTALS_SLUG } from '../data/assessment-definition';

export default function StartApiDeveloperFundamentalsPage() {
  ...
  async function handleStart() {
    ...
    try {
      ...
      setIsStarting(false);
      router.push(
        `/assessments/api-developer-fundamentals/section/${sectionSlug}?attempt=${attempt.id}`
      );
    } catch (startError) {
      setIsStarting(false);
      setError(
    ...
    >
      <div className="mx-auto max-w-3xl rounded-3xl border border-slate-800 bg-slate-900/80 p-8 text-slate-50 shadow-2xl shadow-cyan-950/20">
        ...
        <h1 className="mt-3 text-3xl font-bold">
          APIs para Desarrolladores — Fundamentos
        </h1>
        <p className="mt-3 text-slate-300">
          Vas a completar una sección teórica de 11 preguntas con autosave,
          scoring automático y resultado final consolidado.
        </p>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            type="button"
            onClick={() =>
              router.push('/assessments/api-developer-fundamentals')
            }
