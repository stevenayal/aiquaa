...
import { DATABASE_PRACTICE_SLUG } from '../data/assessment-definition';

export default function StartDatabasePracticePage() {
  ...
  function handleStart() {
    ...
    startTransition(async () => {
      try {
        ...
        router.push(
          `/assessments/database-practice/section/${sectionSlug}?attempt=${attempt.id}`
        );
      } catch (startError) {
        setError(
    ...
    >
      <div className="mx-auto max-w-3xl rounded-3xl border border-slate-800 bg-slate-900/80 p-8 text-slate-50 shadow-2xl shadow-cyan-950/20">
        ...
        <h1 className="mt-3 text-3xl font-bold">
          Bases de Datos — Práctica SQL
        </h1>
        <p className="mt-3 text-slate-300">
          Vas a trabajar sobre una base simulada: predicción de resultados,
          detección de bugs en queries y escritura de SQL, con autosave y
          scoring por sección.
        </p>
        ...
        <div className="mt-8 grid gap-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-5 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-2 text-lg font-semibold">Práctica guiada</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">
              Resultado
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            type="button"
            onClick={() => router.push('/assessments/database-practice')}
