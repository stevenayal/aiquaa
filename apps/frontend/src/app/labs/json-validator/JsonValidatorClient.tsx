...
export default function JsonValidatorClient() {
  ...
  const validateJson = () => {
    try {
      ...
      setIsValid(false);
      const errorMessage =
        err instanceof Error ? err.message : 'Error de sintaxis JSON';
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          >
            Valida, formatea y minifica tu código JSON. Herramienta esencial
              ...
              >
                <div className="flex items-center">
                  {isValid ? (
                    <>
                      <svg
                        className={`w-5 h-5 mr-2 ${isDarkMode ? 'text-green-400' : 'text-green-600'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path
                    ...
                    <>
                      <svg
                        className={`w-5 h-5 mr-2 ${isDarkMode ? 'text-red-400' : 'text-red-600'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path
                ...
                {error && (
                  <p
                    className={`text-sm mt-2 font-mono ${isDarkMode ? 'text-red-300' : 'text-red-700'}`}
                  >
                    {error}
                  </p>
                )}
            ...
            {outputJson && (
              <div className="flex gap-3">
                ...
                >
                  ...
                  >
                    {isFormatted ? 'Formateado' : 'Minificado'} •{' '}
                    {outputJson.length} caracteres
                  </span>
                </div>
              </div>
            )}
