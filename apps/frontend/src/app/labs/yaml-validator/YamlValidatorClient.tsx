...
export default function YamlValidatorClient() {
  ...
  const [mode, setMode] = useState<'validate' | 'format' | 'toJson'>(
    'validate'
  );
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          >
            Valida sintaxis YAML, formatea tu código y convierte entre YAML y
            JSON. Herramienta esencial para DevOps y configuración.
          </p>
        </div>
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  ...
                  {inputYaml && (
                    ...
                    >
                      {inputYaml.split('\n').length} líneas • {inputYaml.length}{' '}
                      caracteres
                    </span>
                  )}
                </div>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                {outputText && !outputText.startsWith('❌') && (
                  <span
                    className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}
                  >
                    {outputText.split('\n').length} líneas • {outputText.length}{' '}
                    caracteres
                  </span>
                )}
              </div>
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <ul
                className={`space-y-2 ${isDarkMode ? 'text-slate-400' : 'text-brand-muted'}`}
              >
        ...
        >
          ...
          <div className="space-y-4">
            ...
            >
              <p
                className={`font-semibold mb-2 ${isDarkMode ? 'text-slate-200' : 'text-gray-900'}`}
              >
                📌 Indentación
              </p>
              <p
                className={`text-sm ${isDarkMode ? 'text-slate-300' : 'text-brand-muted'}`}
              >
                YAML usa espacios (NO tabs) para indentación. Se recomienda 2
                espacios.
              </p>
            </div>
            <div
            ...
            >
              ...
              <p
                className={`text-sm ${isDarkMode ? 'text-slate-300' : 'text-brand-muted'}`}
              >
                No requieren comillas, pero úsalas si contienen caracteres
                especiales o empiezan con números.
              </p>
            </div>
            <div
              className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}
            >
              ...
              <p
                className={`text-sm ${isDarkMode ? 'text-slate-300' : 'text-brand-muted'}`}
              >
                Usa # para comentarios. Todo después de # en la línea es
                ignorado.
              </p>
            </div>
          </div>
