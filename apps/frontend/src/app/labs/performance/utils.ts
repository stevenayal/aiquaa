...
export function loadExamData(): ExamData {
  ...
  const examInfo: ExamInfo = {
    ...
    timeLimit: rawData.metadata.estimated_duration_minutes, // minutos — ExamSimulator multiplica por 60
    pointsPerQuestion: 1
  };
...
export async function exportToPDF(result: ExamResult): Promise<void> {
  ...
  doc.text('AIQUAA | Examen de Performance Testing', pageWidth / 2, yPosition, { align: 'center' });
  yPosition += 10;

  doc.setFontSize(12);
  doc.setTextColor(100, 100, 100);
  doc.text('Fundamentos, Métricas y Herramientas', pageWidth / 2, yPosition, { align: 'center' });
  yPosition += 15;
  ...
  // Download
  doc.save(`Examen-Performance-${result.participantName.replace(/\s+/g, '-')}-${Date.now()}.pdf`);
}
