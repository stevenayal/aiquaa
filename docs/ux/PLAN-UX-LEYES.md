...
**Fecha:** septiembre 2026 · **Rama:** `claude/ux-laws-audit`
...
| **Ley de Postel** | Sé liberal en lo que aceptás, conservador en lo que enviás. | 🔴 El login **nunca** hace `trim()` del email (`LoginForm.tsx:59`): pegar `" ana@mail.com "` desde el gestor de contraseñas devuelve "Credenciales inválidas" y culpa a la contraseña. El campo es `type="text"`, no `type="email"`: en móvil no aparece el teclado con `@`. |
...
| P0-4 | **Modo claro ilegible en assessments.** Tarjetas de pregunta `bg-slate-900/80` fijas, enunciado hereda `text-slate-900`. | `AssessmentSectionScreen.tsx:282,287`; `AssessmentResultScreen.tsx:28` | Estética-Usabilidad | 1 |
| P0-5 | **Envío irreversible sin confirmación.** Sin modal, sin contar preguntas en blanco, mismo tratamiento visual que "continuar". | `AssessmentSectionScreen.tsx:482`; `SectionNavigator.tsx:38` | Fin de Pico, Postel | 2 |
| P0-6 | **Login rechaza emails con espacios.** Sin `trim()`, `type="text"` (no aparece el teclado con `@` en móvil), y el mensaje resultante culpa a la contraseña. | `LoginForm.tsx:59`; `AuthForm.tsx:354`; `validateRegisterForm.ts:50` | **Ley de Postel** | 0,5 |
...
| P4-3 | Extender el *Definition of Done* del repo con la checklist de §7. | Todas |
...
Fase 1  Sprint 1      Resto de P0: modo claro en assessments, confirmación de envío, Postel en login
...
Ampliación del *Definition of Done* de `CLAUDE.md`. Un PR de frontend no se mergea sin esto:
...
- Auditoría de código sobre `apps/frontend` en el commit base de la rama `claude/ux-laws-audit`.
