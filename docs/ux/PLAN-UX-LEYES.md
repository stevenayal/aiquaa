...
| -------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Ley de Hick**            | El tiempo de decisión crece con el número y la complejidad de opciones. | ✅ Rediseñado con el patrón de `/labs`: 4 categorías, destacados y grilla de 3 columnas. Falta el buscador y los filtros (P1-2).                            |
...
Fase 2  En curso      P1: ✅ catálogo, buscador, boundaries y post-registro · falta toasts, cronómetro, a11y de forms
...
| **Buscador y filtros en ambos catálogos.** Texto (normaliza acentos), categoría, nivel y estado. `components/ui/CatalogFilterBar.tsx` concentra la barra: label asociado, `fieldset`/`legend`, `aria-pressed` y contador en `aria-live`. Costo medido en `/assessments`: 199 B → 3,23 kB de JS.                                                    | `components/ui/CatalogFilterBar.tsx` (nuevo), `app/assessments/_components/AssessmentsCatalog.tsx` (nuevo), `app/labs/page.tsx` | Hick, Memoria de Trabajo, Postel              |
| **Catálogo de assessments rediseñado.** 4 categorías, 3 destacados, grilla de 3 columnas y badges de estado (Aprobado / En curso / Mejor puntaje). Nueva fuente única `lib/assessmentsCatalog.ts` + `getMyAssessmentProgressAction`. Sigue siendo Server Component: 199 B de JS.                                                                   | `lib/assessmentsCatalog.ts` (nuevo), `app/assessments/page.tsx`, `actions/assessments.ts`                                       | Hick, Miller, Von Restorff, Posición en Serie |
| **Boundaries de carga y error por sección.** De 2 `loading.tsx` y 3 `error.tsx` a 15 y 15: 89 % de las páginas con estado de carga, 100 % con boundary de error. Dos primitivas (`Skeleton`/`SkeletonScreen` y `SectionError`) en vez de 30 archivos duplicados. Primer uso de `motion-safe:` en el repo y `role="status"` para anunciar la carga. | `components/ui/Skeleton.tsx`, `components/ui/SectionError.tsx` (nuevos) + 26 boundaries                                         | Doherty, Fluir                                |
| **El registro aterriza en `/dashboard`.** Antes caía en `/ranking?welcome=1`, con 0 XP y un cartel que se va a los 6 segundos.                                                                                                                                                                                                                     | `auth/confirm/route.ts`, `auth/confirm-result/page.tsx`                                                                         | Paradoja del Usuario Activo                   |
